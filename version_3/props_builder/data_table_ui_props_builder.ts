...
class DataTableUIPropsBuilder {
    ...
    public static configure(config: DataTableUIBuilderConfig): void {
        ...
        const loader_content =
            loader_html_content ?? (loader_html_content_key ? cm.get<string>(loader_html_content_key, "") : "") ?? "";

        const empty_content =
            empty_data_html_content ??
            (empty_data_html_content_key ? cm.get<string>(empty_data_html_content_key, "") : "") ??
            "";

        DataTableUIPropsBuilder.default_section_id = section_id ?? "DataTableSection";

        DataTableUIPropsBuilder.default_table_id = table_id ?? "DataTable";

        DataTableUIPropsBuilder.defualt_class_styles = class_styles || DataTableUIClassStyles;

        DataTableUIPropsBuilder.default_action_props = action_props;
