<template>
    <div :id="props.id" :class="class_styles.wrapper_class_style" @click="action_handler?.handleOnClick?.($event)">
        <img
            v-if="show_image"
            :src="src"
            :alt="alt_text"
            :loading="boolean_props.lazy_load ? 'lazy' : 'eager'"
            :class="[class_styles.image_class_style, is_loading ? class_styles.loading_class_style : '']"
            @load="action_handler?.handleOnLoad?.()"
            @error="action_handler?.handleOnError?.()"
        />

        <div v-if="content || $slots.default" :class="class_styles.content_wrapper_class_style">
...
const { src, alt_text, content, boolean_props, class_styles } = props;

const { computed_refs, action_handler } = controller;

const { show_image } = computed_refs;

const { is_loading } = controller.state_refs;
</script>
