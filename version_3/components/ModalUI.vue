<template>
    <OverlayUI v-bind="overlay_props">
        <div :class="class_styles.wrapper_class_style" :style="computed_refs.z_index_style.value">
            <transition :name="computed_refs.transition_name.value" appear>
                <div v-if="state_refs.is_visible" :class="class_styles.modal_class_style">
                    <!-- Header -->
                    <div :class="class_styles.header_class_style">
                        <slot name="header">
                            <div :class="class_styles.header_title_wrapper_class_style">
                                ...
                                <h3 :class="class_styles.header_title_class_style" v-html="title_text"></h3>
                            </div>
                            ...
                            <div :class="class_styles.header_close_btn_wrapper_class_style">
                                <button
                                    ...
                                    v-html="
                                        content_props?.close_btn_content ??
                                        getSVGIconValue(content_props?.close_btn_icon_key)
                                    "
                                ></button>
                            </div>
                        </slot>
...
const { overlay_props, title_text, title_icon, title_img, class_styles } = props;

const { state_refs, computed_refs, action_handler, components } = controller;

const { LayoutSectionsUI, OverlayUI } = components;
</script>
