        ...
        <table :id="table_id" :class="class_styles.table_class_style">
                            ...
                            <div v-if="col.sortable" :class="class_styles.sortable_header_wrapper_class_style">
                                <span
                                    :class="[
                                        ...
                                        sort_key === col.key && sort_direction === 'asc' ? 'opacity-100' : 'opacity-30'
                                    ]"
                                ...
                                <span
                                    :class="[
                                        class_styles.sortable_header_icon_class_style,
                                        sort_key === col.key && sort_direction === 'desc' ? 'opacity-100' : 'opacity-30'
                                    ]"
            ...
            <tbody :class="class_styles.tbody_class_style">
                ...
                <tr v-if="props.is_loading">
                    <td :colspan="table_render_obj.length" :class="class_styles?.loading_section_wrapper_class_style">
                        ...
                        <div
                            ...
                            v-for="(_, index) in 12"
                            :key="index"
                            :class="class_styles.loader_skeleton_bar_class_style"
                        ></div>
                    </td>
                </tr>
                ...
                <tr v-else-if="!props.data || props.data.length === 0">
                    <td :colspan="table_render_obj.length" :class="class_styles?.empty_data_wrapper_class_style">
                        <span
                ...
                >
                    ...
                    >
                        <template v-for="cell_content in [action_handler.getCellContent(col, row, index)]" :key="i">
                            ...
                            <span v-else v-html="cell_content"> </span>
                        </template>
                    </td>
                </tr>
            </tbody>
...
const { section_id, table_id, table_render_obj, data, class_styles, is_loading, content } = props;

const { state_refs, action_handler: _action_handler } = controller;

const action_handler = _action_handler as DataTableUIActionHandler<any>;

const { sort_direction, sort_key } = state_refs;
</script>
