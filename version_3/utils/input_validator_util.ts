...
class InputValidatorUtil {
    ...
    private static readonly email_regex_reg_exp =
        /^(?=.{1,255}$)[a-zA-Z0-9._%+-]{1,64}@[a-zA-Z0-9.-]{1,190}\.[a-zA-Z]{2,}$/i;
    private static readonly tel_regex_reg_exp = /^[\s()+-]*([0-9][\s()+-]*){6,20}$/;
    private static readonly pass_regex_reg_exp = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{6,}$/;
    private static readonly url_regex_reg_exp =
    ...
    /** ✅ Check member roles */
    static isAdmin(name: string) {
        return ["SuperAdmin", "AdminI", "AdminT"].includes(name);
    }

    static isSuperAdmin(name: string) {
        return name === "SuperAdmin";
    }

    static isLowerSnakeCase(str: string): boolean {
        return /^[a-z]+(_[a-z]+)*$/.test(str);
    }

    static isUpperPascalCase(str: string): boolean {
    ...
    static isValidNamey(name: string): boolean {
    ...
    static isValidCustomUUID(uuid: string): boolean {
        return this.custom_uuid_regex_reg_exp.test(uuid);
    }

    static isArrayUnique(arr: any[]): boolean {
