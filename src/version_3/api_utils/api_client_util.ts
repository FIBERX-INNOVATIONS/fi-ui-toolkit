...
import { APIRequestConfig, APIClientConfigInterface } from "../types/api_util_type";
...
class APIClient {
    ...
    private attachInterceptors() {
        this.axios_instance.interceptors.request.use(this.setCustomHeaders);

        this.axios_instance.interceptors.response.use((response) => response, this.handleError);
    }

    // Method to Add headers (device ID, device name, token)
    private setCustomHeaders = async (config: InternalAxiosRequestConfig): Promise<InternalAxiosRequestConfig> => {
        try {
    ...
    private handleError = async (error: any) => {
        ...
        if (status === 401 && !original_request._retry) {
            ...
            const success = await RetryStrategyManager.tryRefreshToken();
        ...
        if (status === 409 && !original_request._csrfRetry) {
            ...
            const success = await RetryStrategyManager.tryRefreshCSRF();
