<template>
    <div :class="class_styles.wrapper_class_style">
        <!-- LEFT / TEXT -->
        <div v-if="isTextLeft" :class="class_styles.left_container_class_style">
        ...
        <!-- RIGHT / BUTTON -->
        <div v-if="isTextLeft" :class="class_styles.right_container_class_style">
            <ButtonUI
...
const { class_styles, layout, selection_props } = props;

const { computed_refs, components } = controller;

const { ButtonUI } = components;

const { computed_header_text, show_bulk_button } = computed_refs;
