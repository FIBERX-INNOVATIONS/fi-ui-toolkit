<template>
    <div :class="class_styles.wrapper_class_style">
        ...
        <div
            :class="[
                class_styles.preview_container_class_style,
                state_refs.preview_files.value.length > 1
                    ? class_styles.multiple_file_container_class_style
                    : class_styles.single_file_container_class_style
            ]"
        >
            <div
            ...
            >
                ...
                <!-- IMAGE -->
                <img v-if="item.type === 'image'" :src="item.url" class="max-h-[200px]" />

                <!-- VIDEO -->
                <video v-else-if="item.type === 'video'" controls :src="item.url" class="max-h-[200px]" />

                <!-- DOCUMENT -->
                <iframe v-else-if="item.type === 'document'" :src="item.url" class="w-full h-[200px]" />
