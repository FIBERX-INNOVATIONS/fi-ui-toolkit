...
class RenderHtmlUtil {
    ...
    private static buildIconHtml(icon_key?: SVGIconKey, icon_class?: string): string {
        if (!icon_key) {
            return "";
        }
    ...
    private static combineContent(text: string = "", icon_html: string, order: "icon-first" | "text-first"): string {
        return order === "icon-first" ? `${icon_html}${text}` : `${text}${icon_html}`;
    }
