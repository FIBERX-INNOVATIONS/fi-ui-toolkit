...
class BaseAPIService {
    ...
    protected getStorageKeys() {
        ...
        if (!ACCESS_TOKEN_KEY) {
            missing_keys.push("ACCESS_TOKEN_KEY");
        }

        if (!ACCESS_TOKEN_EXPIRES_AT) {
            missing_keys.push("ACCESS_TOKEN_EXPIRES_AT_KEY");
        }

        if (!DEVICE_ID_KEY) {
            missing_keys.push("DEVICE_ID_KEY");
        }

        if (!DEVICE_NAME_KEY) {
            missing_keys.push("DEVICE_NAME_KEY");
        }
    ...
    private attachInterceptors(): void {
        this.api_instance.interceptors.request.use(async (config: InternalAxiosRequestConfig) => {
    ...
    private async setHeaders(config: InternalAxiosRequestConfig): Promise<InternalAxiosRequestConfig> {
        try {
            ...
            if (valid_token) {
                config.headers["Authorization"] = `Bearer ${valid_token}`;
            }
