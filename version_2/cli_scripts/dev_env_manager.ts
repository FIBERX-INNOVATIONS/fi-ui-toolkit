...
class DevEnvManager {
    ...
    private writeENVFile(): void {
        const file_content = Object.entries(this.env_data)
    ...
    private async promptNewKeys(): Promise<void> {
        ...
        const keys = keys_input
            .split(",")
            .map((k) => k.trim())
            .filter(Boolean);

        for (const key of keys) {
            const formatted_key = key.toUpperCase().startsWith("VITE_")
                ? key.toUpperCase()
    ...
    public async run(): Promise<void> {
        try {
            ...
            if (confirmed) {
                ...
                this.logger.log("❌ Operation canceled.");
            }
        } catch (error: unknown) {
